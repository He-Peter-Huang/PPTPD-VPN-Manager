...
def Main():
	while(True):	
		...
		if (select=="1"):
			clear()
			os.system("python3 /etc/pptp-vpn-manager/adduser")
		if (select=="2"):
			clear()
			os.system("python3 /etc/pptp-vpn-manager/checkuser")
		if (select=="3"):
			clear()
			os.system("last | grep still | grep ppp")
		if (select=="4"):
			clear()
			os.system("nano /etc/pptp-vpn-manager/chap-secrets")
		if (select=="5"):
			clear()
			os.system("last |grep ppp")
		if (select=="6"):
			clear()
			os.system("python3 /etc/pptp-vpn-manager/loginhistory")
